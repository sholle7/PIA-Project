...
export class RegisterComponent implements OnInit {
  ...
  errorMessage(err) {
    this.notificationService.error('Neuspesna registracija ', err, {
  ...
  successMessage(success) {
    this.notificationService.success('Uspesna registracija', success, {
      position: ['bottom', 'right'],
      timeOut: 3000,
      animate: 'fade',
    })
  ...
  this.userService.usernameExists(this.username).subscribe((user: User)=>{
    ...
    this.userService.emailExists(this.email).subscribe((user:User)=>{
      ...
      this.userService.register(this.username, this.password, this.fullName, this.address, this.email, this.telephone, this.imgURL,this.status,this.type).subscribe((res)=>{
        if (res['message'] == 'ok') {
          this.successMessage("Uspesno je poslat zahtev za registraciju");
        }
        else {
          this.errorMessage("Neuspesno je poslat zahtev za registraciju");
        }
      })
    })
  })
